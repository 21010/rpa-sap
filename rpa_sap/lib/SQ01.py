...
class SQ01:
    ...
    def start_query(
        ...
        if user_group:
            ...
            self.interactor.press_button("wnd[0]/tbar[1]/btn[19]")

            # press Filter button
            self.interactor.press_button("wnd[1]/tbar[0]/btn[29]")
            ...
            # Press < button
            self.interactor.press_button(
                "wnd[2]/usr/subSUB_DYN0500:SAPLSKBH:0600/btnAPP_WL_SING"
            )

            # Press Set Value button
            self.interactor.press_button(
                "wnd[2]/usr/subSUB_DYN0500:SAPLSKBH:0600/btn600_BUTTON"
            )
            ...
            # Press Search Button
            self.interactor.press_button("wnd[3]/tbar[0]/btn[0]")

            # Select User Group row
            try:
                self.interactor.set_property(
                    "wnd[1]/usr/cntlGRID1/shellcont/shell", "selectedRows", 0
                )
            ...
            # Press OK button
            self.interactor.press_button("wnd[1]/tbar[0]/btn[0]")

        # Enter the query name
        self.interactor.set_text("wnd[0]/usr/ctxtRS38R-QNUM", query_name)

        if variant_name:
            # Press Execute with Variants button
            self.interactor.press_button("wnd[0]/tbar[1]/btn[17]")
            # Enter variant name
            self.interactor.set_text("wnd[1]/usr/ctxtRS38R-VARIANT", variant_name)
            # Press OK button
            self.interactor.press_button("wnd[1]/tbar[0]/btn[0]")
            ...
            self.interactor.press_button("wnd[0]/tbar[1]/btn[8]")
    ...
    def to_local_file(self, folder_path: str, file_name: str, file_type: str = "xls"):
        # Press Export button
        self.interactor.press_button("wnd[0]/tbar[1]/btn[45]")
        # Select export type
        ...
        self.interactor.press_button("wnd[1]/tbar[0]/btn[0]")
        # enter folder path
        self.interactor.set_text("wnd[1]/usr/ctxtDY_PATH", folder_path)
        # enter file name
        self.interactor.set_text("wnd[1]/usr/ctxtDY_FILENAME", file_name)
        # set code pate
        self.interactor.set_text(
            "wnd[1]/usr/ctxtDY_FILE_ENCODING",
            "0000" if file_type == "xls" else "0004" if file_type == "csv" else "0000",
        )
        # Press OK button
        self.interactor.press_button("wnd[1]/tbar[0]/btn[11]")
        # Verify status bar
        statusbar = self.interactor.get_status_bar_message()
        if "Download" not in statusbar.text:
            raise Exception("Data has not been exported sucessully")
