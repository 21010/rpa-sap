...
class SapSession:
    ...
    def execute_rfc(self, function_name: str):
        ...
        if not self.sap_functions:
            ...
            conn_string = self._rfc_credentials["connection_string"]
            import re

            match = re.search(r"/H/([^/]+)/S/(32\d{2})", conn_string)
            ...
            try:
                ...
                raise SapSessionError(
                    f"Failed to establish dynamic RFC connection: {e}"
                ) from e
